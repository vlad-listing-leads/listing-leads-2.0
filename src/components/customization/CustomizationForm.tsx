...
export function CustomizationForm({
  ...
  // Add original template entry on mount (for revert to original)
  useEffect(() => {
    setPromptHistory(prev => {
      // Check if we already have an "Original template" entry
      const hasOriginalEntry = prev.some(item => item.id === 'original-template')
      if (hasOriginalEntry) return prev

      const originalEntry: PromptHistoryItem = {
        id: 'original-template',
        prompt: 'Original template loaded',
        timestamp: new Date(),
        type: 'system',
        htmlSnapshot: template.html_content
      }
      return [originalEntry, ...prev]
    })
